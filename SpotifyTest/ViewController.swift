    ...
    override func viewDidAppear(_ animated: Bool) {
     ...
     //       print("123: \(SPTAudioStreamingController.sharedInstance().playbackState.accessibilityActivate())")
            ...
            if restart == true{
                ...
                let mainImageURL = URL(string: songList[0].mainImage! as String)
                ...
                if(songList.count == 0){
                    ...
                    for aSong in songList{
                        let newSong : NSDictionary = [
                            "artist" : aSong.artistName,
                            "name" : aSong.name,
                            "track" : aSong.trackU,
                            "likes" : aSong.likes,
                            "image" : aSong.mainImage
                        ]
                        myArray.append(newSong)
                    }
                    let someArray = myArray as! NSArray
                    let descriptor: NSSortDescriptor = NSSortDescriptor(key: "likes", ascending: false)
                    let sortedResults: NSArray = someArray.sortedArray(using: [descriptor]) as NSArray
...
//        print("Source \(SPTAudioStreamingController.sharedInstance().metadata.currentTrack?.playbackSourceUri)")
 ...
 //               print("*** failed to play: \(error)")
        ...
        let myMainImage = UIImage(data: mainImageData! as Data)
        npImage = myMainImage
        let npCell : NSDictionary = [
            "artist" : songList[0].artistName,
            "image" : songList[0].mainImage,
            "name" : songList[0].name
        ]
        songList.remove(at: 0)
        let ref = FIRDatabase.database().reference()
        let hostRef = ref.child("hosts").child(hostName)
        ref.child("NowPlaying").child(hostName).setValue(npCell)
        if(songList.count == 0){
            hostRef.setValue(1)
        }
        else{
